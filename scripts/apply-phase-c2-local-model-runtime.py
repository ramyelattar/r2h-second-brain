...
MODULE = SRC / "local_model_runtime.rs"
LIB = SRC / "lib.rs"
COMMAND_TEST = TAURI / "tests" / "commands.rs"
...
def backup(path: Path) -> None:
    target = path.with_name(path.name + ".before-phase-c2")
    if path.exists() and not target.exists():
        target.write_bytes(path.read_bytes())


module_source = r'''use std::{
    fs::{self, OpenOptions},
    io::{self, Read, Write},
...
const MODEL_ALIAS: &str = "qwen3-4b-r2h";
...
impl LocalModelRuntimeManager {
    ...
    fn start_owned_runtime(&self) -> io::Result<()> {
        ...
        let server = self.config.server_path();
...
fn sanitize_error(error: &io::Error) -> String {
    match error.kind() {
        io::ErrorKind::NotFound => {
            "Local Qwen runtime component is missing".to_owned()
        }
        io::ErrorKind::TimedOut => {
            "Local Qwen runtime startup timed out".to_owned()
        }
        io::ErrorKind::AlreadyExists => {
            "Local Qwen runtime is already managed".to_owned()
        }
...
lib = replace_once(
    ...
    commands + "pub fn run() -> Result<(), Box<dyn std::error::Error>> {",
...
lib = replace_once(
    lib,
    '''        ) && let Some(runtime) = app_handle.try_state::<KhojRuntimeManager>()
        {
            let _ = runtime.stop();
        }''',
    '''        ) {
            if let Some(runtime) =
                app_handle.try_state::<LocalModelRuntimeManager>()
            {
                let _ = runtime.stop();
            }

            if let Some(runtime) =
                app_handle.try_state::<KhojRuntimeManager>()
            {
                let _ = runtime.stop();
            }
        }''',
...
tests = COMMAND_TEST.read_text(encoding="utf-8")
backup(COMMAND_TEST)

tests = replace_once(
    tests,
    "const APPROVED_COMMANDS: [&str; 17] = [",
...
COMMAND_TEST.write_text(tests, encoding="utf-8", newline="\n")

print(f"CREATED {MODULE}")
print(f"UPDATED {LIB}")
print(f"UPDATED {COMMAND_TEST}")
print("PHASE_C2_LOCAL_MODEL_RUNTIME_WRITTEN")
